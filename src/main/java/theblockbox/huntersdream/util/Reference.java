package theblockbox.huntersdream.util;

public class Reference {
    public static final String MODID = "huntersdream";
    public static final String NAME = "Hunter's Dream";
    public static final String VERSION = "BUILD_VERSION"; // This is set during build from gradle.properties
    public static final String CLIENT = "theblockbox." + Reference.MODID + ".proxy.ClientProxy";
    public static final String SERVER = "theblockbox." + Reference.MODID + ".proxy.ServerProxy";
    public static final String ENTITY_TEXTURE_PATH = Reference.MODID + ":textures/entity/";
    public static final String MC_VERSION = "1.12.2";
    public static final String UPDATE_JSON = "https://raw.githubusercontent.com/TheGamingLord/HuntersDream/master/updatechecker.json";
    /**
     * Config lang key
     */
    public static final String CFG_LANG = Reference.MODID + ".config.";
}